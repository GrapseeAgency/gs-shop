...
function DealCountdown({ endTime }: { endTime: Date }) {
  const endTs = useMemo(() => endTime.getTime(), [endTime])
  const timeLeft = useCountdown(endTs)
...
function DealCard({ product, index, dealType }: { product: Product; index: number; dealType: string }) {
  ...
  return (
    ...
    >
      ...
      {/* Image */}
      <div
        className="relative flex w-full items-center justify-center overflow-hidden bg-gradient-to-br from-destructive/10 via-orange-500/5 to-transparent cursor-pointer"
...
export function DealsView() {
  ...
  // Stable end-of-day timestamp computed once per mount
  const endTs = useMemo(() => {
    const d = new Date()
  ...
  useEffect(() => {
    const fetchDeals = async () => {
      setLoading(true)
      try {
        // Fetch flashback deals first from the real API
        const flashRes = await fetch('/api/flashback')
        let dealProducts: Product[] = []
        if (flashRes.ok) {
          const flashbackData = await flashRes.json()
          if (Array.isArray(flashbackData)) {
            dealProducts = flashbackData.map((fb: any) => ({
              ...fb.product,
              price: fb.product.price * (1 - fb.discount / 100),
              comparePrice: fb.product.price,
              discount: fb.discount,
              isFlashDeal: true,
              flashDealEnds: fb.flashEnd
            }))
          }
        }

        // If we have flashback deals, use them. Otherwise fallback to all products with comparePrice
