...
export async function POST(req: NextRequest) {
  try {
    ...
    if (card.rewardType === 'points') {
      ...
      couponCode = `SCRATCH${Math.random().toString(36).substring(2, 8).toUpperCase()}`
      
      await prisma.coupon.create({
        data: {
          code: couponCode,
          discount: card.rewardValue,
          discountValue: card.rewardValue,
          type: 'percentage',
          maxUses: 1,
      ...
      couponCode = `FREESHIP${Math.random().toString(36).substring(2, 6).toUpperCase()}`
