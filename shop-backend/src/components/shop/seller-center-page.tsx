...
interface SellerData {
  ...
  products: { id: string; name: string; price: number; sales: number; rating: number; status: string }[]
...
export function SellerCenterPage() {
  ...
  useEffect(() => {
    const fetchSellerData = async () => {
      setLoading(true)
      try {
        const res = await fetch('/api/seller-center')
        if (res.ok) {
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success('Application submitted!', { description: 'We\'ll review your application within 24 hours.' })
        setShowApply(false)
        setStoreName('')
        setEmail('')
      }
  ...
  const currentTier = data.tiers.find(t => t.name.toLowerCase() === data.tier) || data.tiers[2]
  ...
  return (
    <motion.div className="min-h-screen bg-background pb-6" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3 }}>
      ...
      <div className="px-4 mt-3 grid grid-cols-2 gap-2">
        {[
          { icon: ShoppingBag, label: 'Total Sales', value: data.stats.totalSales.toLocaleString(), color: 'sky' },
          { icon: DollarSign, label: 'Revenue', value: formatPrice(data.stats.totalRevenue), color: 'emerald' },
          { icon: Star, label: 'Rating', value: data.stats.rating.toString(), color: 'amber' },
          { icon: Package, label: 'Products', value: data.stats.activeProducts.toString(), color: 'violet' },
        ].map((stat, i) => (
          <motion.div
            key={i}
      ...
      <div className="px-4 mt-4">
        ...
        <p className="text-[10px] text-muted-foreground mt-2">Commission rate: <span className="text-amber-500 font-medium">{data.commissionRate}%</span></p>
      </div>
      ...
      <div className="px-4 mt-4">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="text-[11px] text-muted-foreground">{data.stats.pendingOrders} pending</span>
        </div>
          ...
          {data.recentOrders.map((order, i) => (
            ...
            >
              ...
              <div className="text-right">
                <p className="text-xs font-bold text-foreground">{formatPrice(order.amount)}</p>
