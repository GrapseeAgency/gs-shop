  ...
  try {
    ...
    if (!summary) {
      ...
      const summaryText = total > 0 
        ? `${positivePercent}% of customers had a positive experience with this product. ${positive} out of ${total} reviews.`
        : 'No reviews yet.'
...
export async function POST(req: NextRequest) {
  try {
    ...
    const summaryText = total > 0 
      ? `${positivePercent}% of customers had a positive experience. ${positive} positive reviews.`
      : 'No reviews yet.'
