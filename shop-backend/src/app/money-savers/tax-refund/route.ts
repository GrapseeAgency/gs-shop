  ...
  try {
    ...
    for (const order of orders) {
      for (const item of order.items) {
        ...
        if (tags.includes('medicine') || tags.includes('health') || tags.includes('medical')) {
          categories.medical.amount += item.price * item.quantity
          categories.medical.items.push(product)
        }

        if (tags.includes('book') || tags.includes('education') || tags.includes('study')) {
          categories.education.amount += item.price * item.quantity
          categories.education.items.push(product)
        }

        if (tags.includes('office') || tags.includes('furniture') || tags.includes('work')) {
