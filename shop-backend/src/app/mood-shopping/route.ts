...
function calculateMoodMatch(product: any, moodConfig: any[]) {
  ...
  // Category match gives significant bonus
  moodConfig.forEach(config => {
    const categoryName = product.category?.name?.toLowerCase() || ''
    const productTags = (product.tags || '').toLowerCase()

    // Check category match (max 20 points per config)
    if (categoryName.includes(config.category.toLowerCase())) {
      bonus += 20
    }

    // Check tag matches (max 5 points per tag, capped at 15 per config)
    let tagBonus = 0
    config.tags.forEach((tag: string) => {
      if (productTags.includes(tag.toLowerCase()) && tagBonus < 15) {
        tagBonus += 5
      }
    })
    bonus += tagBonus
  ...
  // Add rating bonus (up to 10 points for highly rated products)
  ...
  // Cap the bonus and calculate final score
  const cappedBonus = Math.min(maxBonus, bonus)
  return Math.min(100, score + cappedBonus)
}
