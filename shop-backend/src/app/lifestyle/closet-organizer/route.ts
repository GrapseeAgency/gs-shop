  ...
  try {
    ...
    for (const order of orders) {
      for (const item of order.items) {
        ...
        // Check if clothing item by tags
        const isClothing = product.tags?.toLowerCase().match(/clothing|fashion|apparel|wear/)
        if (isClothing) {
          clothingItems.push({
            id: item.id,
            name: product.name,
...
function generateOrganizationSuggestions(items: any[]) {
  ...
  const dominantColor = Object.entries(colors).sort((a, b) => (b[1] as number) - (a[1] as number))[0]
  
  if (dominantColor && (dominantColor[1] as number) > items.length * 0.4) {
    suggestions.push({
