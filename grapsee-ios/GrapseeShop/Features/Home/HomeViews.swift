...
struct HomeDestination: View {
    ...
    var body: some View {
        ...
        case .aiscoper: AiScoperView()
        case .aicompetitor: AiCompetitorView()
        case .aipreview: AiPreviewView()
        case .aiproposal: AiProposalView()
        case .freeaudit: FreeAuditView()
        case .qualitycert: QualityCertificateView()
        case .portfolioproof: PortfolioProofView()
        case .casestudies: CaseStudiesView()
        case .emergencybuy: EmergencyQuickBuyView()
        case .clipboardbuy: ClipboardPurchaseView()
        case .flashback: FlashbackDealsView()
        case .expiryguar: ExpiryGuaranteeView()
        case .priceguar: PriceGuaranteeView()
        case .smartupsell: SmartUpsellView()
        case .studentbudget: StudentBudgetView()
        case .subexpiry: SubscriptionExpiryView()
        case .altfinder: AlternativeFinderView()
        case .assembly: AssemblyFinderView()
        case .autocoupon: AutoCouponView()
        case .bulkbuy: BulkBuyView()
        case .reorder: OneClickReorderView()
        case .pricedroprefund: PriceDropRefundView()
        case .grocery: GroceryImportView()
        case .recipe: RecipeToCartView()
        case .petsup: PetSuppliesView()
        case .schoolsup: SchoolSuppliesView()
        case .movingkit: MovingKitView()
        case .appliance: ApplianceRepairView()
        case .safety: SafetyRecallView()
        case .verifiedphotos: VerifiedPhotosView()
        case .childgrowth: ChildGrowthView()
        case .diabetic: DiabeticScannerView()
        case .ingscan: IngredientScannerView()
        case .ingswap: IngredientSwapView()
        case .medinteract: MedicineInteractionView()
        case .medtracker: MedicineTrackerView()
        case .rxscan: PrescriptionScanView()
        case .healthmon: HealthMonitorView()
        case .projdash: ProjectDashboardView()
        case .projplan: ProjectPlannerView()
        case .milestones: MilestonesView()
        case .deadline: DeadlinePredictorView()
        case .scopechange: ScopeChangeView()
        case .handoff: HandoffPortalView()
        case .slagen: SlaGeneratorView()
        case .qbr: QbrReportsView()
        case .churn: ChurnPredictionView()
        case .clientltv: ClientLtvView()
        case .command: CommandCenterView()
        case .corpcredit: CorporateCreditView()
        case .crowd: CrowdWisdomView()
        case .demand: DemandForecastView()
        case .dispute: DisputeResolutionView()
        case .vault: GuaranteeVaultView()
        case .pricetest: PricingTestView()
        case .liquidator: ProductLiquidatorView()
        case .profit: ProfitabilityView()
        case .scheduler: SchedulerView()
        case .seasonal: SeasonalClothingView()
        case .senior: SeniorModeView()
        case .svcconfig: ServiceConfiguratorView()
        case .svcsubs: ServiceSubscriptionsView()
        case .subaudit: SubscriptionAuditView()
        case .eco: SustainableFinderView()
        case .ussd: UssdMenuView()
        case .videoverify: VideoVerificationView()
        case .vision: VisualImpairedView()
        case .warrantyexp: WarrantyExpiryView()
        case .wabulk: WhatsAppBulkView()
        ...
        case .bodytype: BodyTypeView()
        case .usecasematcher: UseCaseMatcherView()
        case .wardrobe: WardrobePlannerView()
        case .revtokens: RevisionTokensView()
        case .trendforecaster: TrendForecasterView()
        case .eventstylist: EventStylistView()
                ...
                if state.cart.isEmpty {
                    ...
                    List {
                        ForEach(state.cart) { line in
                            HStack { VStack(alignment: .leading) { Text(line.name).lineLimit(1); Text("Qty \(line.quantity)").font(.caption).foregroundColor(.secondary) }; Spacer(); Text("$\(String(format: "%.2f", line.price * Double(line.quantity)))").bold() }
                        }
