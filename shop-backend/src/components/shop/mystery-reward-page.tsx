...
export function MysteryRewardPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch('/api/mystery-reward')
        if (res.ok) {
  ...
  const revealReward = async () => {
    ...
    const weights = [25, 20, 8, 15, 5, 10, 6, 3, 2, 1]
    ...
    const selected = possibleRewards[selectedIndex]
    setReward(selected)
    setRevealed(true)
    setMysteryRewardClaimed(true)

    if (selected.type === 'points') {
      addRewardsPoints(selected.value as number)
    }
