...
export function PriceDropPage() {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (data.products?.length) {
            ...
            const initiallyWatched = data.products.filter((p: any) => p.watched).map((p: any) => p.id)
            setWatched(new Set(initiallyWatched))
          }
        }
      } catch { /* silent */ }
      setLoading(false)
  ...
  const filtered = useMemo(() => {
    ...
    switch (sortBy) {
      ...
      case 'newest': result.sort((a, b) => new Date(b.dropDate).getTime() - new Date(a.dropDate).getTime()); break
      case 'price-low': result.sort((a, b) => a.currentPrice - b.currentPrice); break
    }
    return result
  }, [products, dropFilter, sortBy])

  const toggleWatch = async (product: PriceDropProduct) => {
    try {
      const res = await fetch('/api/price-drop', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ productId: product.id, productName: product.name, price: product.currentPrice }),
      })
      if (res.ok) {
        const data = await res.json()
        setWatched(prev => {
            ...
            {filtered.map((product, index) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Watch Button */}
                  <button onClick={() => toggleWatch(product)} className="absolute top-2 right-2 z-10 flex h-6 w-6 items-center justify-center rounded-full bg-background/60 backdrop-blur-sm">
