  ...
  const handlePurchase = async (productId: string) => {
    try {
      const res = await fetch('/api/digital-downloads', {
        ...
        body: JSON.stringify({ productId, action: 'purchase' }),
      })
      if (res.ok) {
        toast.success('Product purchased!', { description: 'You can now download it from My Downloads.' })
        setProducts(prev => prev.map(p => p.id === productId ? { ...p, isPurchased: true } : p))
      }
    } catch {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setTimeout(() => {
          ...
          setDownloadProgress(0)
          toast.success('Download complete!', { description: `${data.downloadsRemaining} downloads remaining` })
          setMyDownloads(prev =>
            prev.map(d => d.productId === productId ? { ...d, downloadsRemaining: data.downloadsRemaining } : d)
          )
        }, 1500)
      }
    } catch {
