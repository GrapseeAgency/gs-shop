...
const tierConfig: Record<BadgeTier, { label: string; icon: React.ElementType; color: string; bg: string }> = {
  new: { label: 'New', icon: User, color: 'text-gray-400', bg: 'bg-gray-500/10' },
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-2 mt-0.5">
                <Badge className={`${tierConfig[reviewerOfMonth.badgeTier].bg} ${tierConfig[reviewerOfMonth.badgeTier].color} text-[9px] gap-0.5`}>
                  {(() => { const TierIcon = tierConfig[reviewerOfMonth.badgeTier].icon; return <TierIcon className="h-2.5 w-2.5" /> })()}
                  {tierConfig[reviewerOfMonth.badgeTier].label}
                </Badge>
              </div>
      ...
      <div className="mx-4 mt-4">
        ...
        <div className="space-y-2">
          {reviewers.map((reviewer, i) => {
            const cfg = tierConfig[reviewer.badgeTier]
            const TierIcon = cfg.icon
            return (
              <motion.div
                key={reviewer.id}
      ...
      <div className="mx-4 mt-4">
        ...
        <div className="space-y-2">
          {viralReviews.map((review, i) => {
            const cfg = tierConfig[review.badgeTier]
            return (
