...
interface TierInfo {
  ...
  multiplier: number
  discount: number
  freeDeployment: boolean
  icon: React.ComponentType<{ className?: string }>
  color: string
}

const TIERS: TierInfo[] = [
  { name: 'Bronze', minPoints: 0, multiplier: 1.0, discount: 0, freeDeployment: false, icon: Medal, color: 'from-amber-700/20 to-amber-800/10' },
  { name: 'Silver', minPoints: 1000, multiplier: 1.1, discount: 5, freeDeployment: false, icon: Medal, color: 'from-gray-400/20 to-gray-500/10' },
  { name: 'Gold', minPoints: 5000, multiplier: 1.2, discount: 10, freeDeployment: true, icon: Award, color: 'from-yellow-400/20 to-yellow-600/10' },
  { name: 'Platinum', minPoints: 15000, multiplier: 1.3, discount: 15, freeDeployment: true, icon: Diamond, color: 'from-cyan-400/20 to-cyan-600/10' },
  { name: 'Diamond', minPoints: 50000, multiplier: 1.5, discount: 20, freeDeployment: true, icon: Crown, color: 'from-blue-400/20 to-indigo-400/10' },
]

const REDEMPTION_CATALOG = [
  { points: 200, reward: '50 BDT Off Coupon', icon: Ticket },
  { points: 400, reward: '100 BDT Off Coupon', icon: Ticket },
  { points: 300, reward: '1-Mo Private Repo Update Sync', icon: Laptop },
  { points: 600, reward: 'Birthday Bonus Points', icon: PartyPopper },
  { points: 800, reward: '250 BDT Off Coupon', icon: Gift },
  { points: 1500, reward: 'FREE Professional Cloud Setup', icon: Cloud },
        ...
        <div className="overflow-x-auto -mx-4 px-4">
          <table className="w-full text-xs border border-border rounded-lg overflow-hidden">
            <thead><tr className="bg-muted/50">
              ...
              <th className="p-2 text-center text-muted-foreground font-medium">Setup</th>
            </tr></thead>
            <tbody>
              {TIERS.map((t) => (
                <tr key={t.name} className={`border-t border-border ${t.name === currentTier.name ? 'bg-amber-500/10' : ''}`}>
                  ...
                  <td className="p-2 text-center">{t.freeDeployment ? <Check className="h-4 w-4 text-green-500 mx-auto" /> : ''}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          {showHowItWorks && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="mt-3 space-y-2">
              {[
                ...
                { icon: '', text: 'Redeem points for coupons & cloud setup service' },
