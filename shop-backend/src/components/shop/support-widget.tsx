...
const []: SupportTicket[] = [
  { id: 'TK-1042', subject: 'Delivery delayed for order #3829', status: 'in-progress', updatedAt: '2 hours ago' },
  { id: 'TK-1038', subject: 'Refund request for Brand Identity Design', status: 'open', updatedAt: '1 day ago' },
  { id: 'TK-1030', subject: 'Wrong item received in order #3756', status: 'resolved', updatedAt: '3 days ago' },
]
...
export function SupportWidget() {
  ...
  const menuActions = [
    ...
    { icon: Ticket, label: 'My Tickets', desc: `${[].filter(t => t.status === 'open' || t.status === 'in-progress').length} active`, color: 'from-sky-500 to-blue-600', action: () => setActiveTab('tickets') },
            ...
            {expanded && (
              ...
              >
                ...
                <div className="flex items-center justify-between bg-gradient-to-r from-primary to-primary/80 p-3">
                  ...
                  <div className="flex items-center gap-1">
                    <button onClick={() => setExpanded(false)} className="rounded-full p-1 text-white/60 hover:text-white hover:bg-white/10">
                      <Minimize2 className="h-3.5 w-3.5" />
                    </button>
                    <button onClick={() => { setExpanded(false); setActiveTab('menu') }} className="rounded-full p-1 text-white/60 hover:text-white hover:bg-white/10">
