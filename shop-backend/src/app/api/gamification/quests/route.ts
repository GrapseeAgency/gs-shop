...
export async function GET(req: NextRequest) {
  try {
    ...
    const questsWithStatus: any[] = QUESTS.map(quest => {
      ...
      return {
        ...
        progress: userProgress?.currentCount || 0,
        percent: userProgress 
          ? Math.min(100, Math.round((userProgress.currentCount / quest.requirement.count) * 100))
          : 0
      }
    })
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Check if completed
    const isCompleted = progress.currentCount >= quest.requirement.count
    ...
    return NextResponse.json({
      ...
      percent: Math.round((progress.currentCount / quest.requirement.count) * 100)
    })
  } catch (error) {
